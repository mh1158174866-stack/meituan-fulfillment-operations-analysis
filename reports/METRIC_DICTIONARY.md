...
- 分母：订单数。
- 单位：次/订单。
- 过滤条件：保留全量订单；质量异常另计不静默删除。
- 0/NULL处理：分母0则NULL。
- 质量标记与适用边界：含最终接受尝试；61条属性冲突按最终接受waybill归属。
...
- 过滤条件：保留全量订单；质量异常另计不静默删除。
- 0/NULL处理：分母0则NULL。
- 质量标记与适用边界：首次尝试不算重派；61条属性冲突按最终接受waybill归属。
...
- 质量标记与适用边界：1条未完成保留在分母；61条属性冲突按最终接受waybill归属。
...
- 分母：订单数。
- 单位：比例。
- 过滤条件：保留全量订单；61条属性冲突按最终接受waybill归属。
- 0/NULL处理：分母0则NULL。
- 质量标记与适用边界：第一次尝试由dispatch_time排序确定；0时间排末尾并标记。
...
- 分母：不适用。
- 单位：订单。
- 过滤条件：保留全量订单；时间异常另计quality_excluded_order_count。
- 0/NULL处理：无行则为0。
- 质量标记与适用边界：dt为最终接受waybill的运营归属日；61条属性冲突保留。
...
- 分母：全部waybill尝试数。
- 单位：比例。
- 过滤条件：保留全量waybill；时间异常另计quality_excluded_waybill_count。
- 0/NULL处理：分母0则NULL。
